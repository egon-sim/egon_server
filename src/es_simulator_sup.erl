...
start_link(SimId, Connection) ->
   io:format("Starting new simulator (Id: ~p).~n", [SimId]),
   supervisor:start_link({local, ?SERVER}, ?MODULE, [SimId, Connection]).

init([SimId, Connection]) ->
   ...
   Interface = {es_interface_server, {es_interface_server, start_link, [SimId, Connection]},
