-module(es_action_server).
-behaviour(gen_server).
-export([start_link/1, init/1, handle_call/3, handle_cast/2, handle_info/2, terminate/2, code_change/3]).
-record(action_state, {port, lsock, buffer}).
...
%    io:format("server listening...~n"),
    {ok, #action_state{port = Port, lsock = LSock, buffer=[]}, 0}.
...
handle_info(timeout, #action_state{lsock = LSock} = State) ->
...
%    io:format("~p~n", [RawData]),
    Buffer = State#action_state.buffer,
    {Newline, [CleanData]} = re:run(RawData, "^([^\\R]+)\\R+$", [{capture, [1], list}]),
    if
        Newline =:= match ->
	    New_state = State#action_state{buffer = Buffer ++ CleanData},
	    exec_call(New_state, _Socket),
	    S1 = State#action_state{buffer = []};
	true ->
	    S1 = State#action_state{buffer = Buffer ++ RawData}
    end,
    S1.
